...
$(document).ready(function () {
    ...
    $('#sendMailBtn').click(function () {
        if (!valid_email_address($("#email").val())) {
            ...
            $.ajax({
                ...
                data: {'username':$('#username').val(),'email':$('#email').val(),'phoneNo':$('#phoneNo').val(),'message':$('#message').val()},
                type: 'POST',
                success: function () {
                    swal({
                        type: 'success',
                        html: '恭喜您申请成功，我们将尽快与您取得联系！'
                    });
                    $('#username').val('');
                    $('#email').val('');
                    $('#phoneNo').val('');
    ...
    if (typeof CountUp === "function") {
        ...
        /*-------------Index---------------*/
        if ($("#Investors")[0]) {
            var demo4 = new CountUp("Success", 0, 30, 0, 7, options);
            var demo3 = new CountUp("Client", 0, 78, 0, 7, options);
            ...
            $(window).on('scroll', function () {
                ...
                var animation1 = $('#Investors').offset().top;
